...
Works on any Python/sklearn version (no ColumnTransformer)
"""
...
def load_artifacts():
    ...
    xgb = XGBRegressor()
    xgb.load_model(os.path.join(base, 'xgb_model.json'))
    ...
    return scaler, ridge, rf, xgb, meta, feat['columns']

try:
    scaler, ridge, rf, xgb, meta, feat_cols = load_artifacts()
...
if st.button("🔮 Predict Average Score", type="primary", use_container_width=True):
    ...
    input_data = pd.DataFrame([[raw.get(c, 0) for c in feat_cols]], columns=feat_cols)
